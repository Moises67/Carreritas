class Game {
  ...
  handleResetButton() {
    this.resetButton.mousePressed(()=>{database.ref("/").set({playerCount:0, gameStatus:0, players:{}});window.location.reaload()});
    update();
  }
  ...
  handlePlayerControls() {
    ...
    if (keyIsDown(LEFT_ARROW) && player.positionX>=width*0.3) {
      player.positionX -= 10;
      player.update();
    }
    if (keyIsDown(RIGHT_ARROW) && player.positionX<=width*0.8) {
      player.positionX += 10;
      player.update();
    }
  }
